...
def estimate_total_num_articles(files_to_tokenize, split_dir):
    """Estimates the total number of articles based on number of articles in first split times number of splits.
    
    Counts only non-empty lines to get an accurate estimate.
    ...
    with open(os.path.join(split_dir, files_to_tokenize[0]), "r", encoding="utf-8", errors="replace") as file:
        for line in file:
            # Count only non-empty lines (actual articles)
            if line.strip():
                lines_per_file += 1
    ...
    with alive_bar(total_num_articles) as bar:
        while True:
            for i, future in enumerate(futures):
                if future.done() and future not in finished_futures:
                    try:
                        ...
                        indiv_metric = future.result(timeout=1)
            ...
            if all(future.done() for future in futures):
                ...
                # Update progress bar to 100% to show completion
                with num_tokenized_articles_lock:
                    actual_total = num_tokenized_articles.value
                    remaining = max(0, actual_total - bar.current)
                    if remaining > 0:
                        bar(remaining)
                break
            
            # Check for hung processes (no progress for a long time)
            elapsed_time = time.time() - tokenization_start_time
            with num_tokenized_articles_lock:
                current_progress = num_tokenized_articles.value
                # Track if we've made progress since last check
                progress_made = current_progress > prev_num_tokenized_articles
            
            # Track time since last progress
            if not hasattr(multiprocess_data_prep, '_last_progress_time'):
                multiprocess_data_prep._last_progress_time = tokenization_start_time
            if progress_made:
                multiprocess_data_prep._last_progress_time = time.time()
            
            time_since_progress = time.time() - multiprocess_data_prep._last_progress_time
            running_count = sum(1 for f in futures if not f.done())
            
            # If no progress for 2 minutes, log warning
            if time_since_progress > 120 and current_progress < total_num_articles:
                LOGGER.warning(
                    f"Warning: No progress for {int(time_since_progress)}s. "
                    f"Progress: {current_progress}/{total_num_articles} ({len(finished_futures)}/{len(futures)} processes finished, {running_count} still running)."
                )
            
            # If no progress for 5 minutes, raise error
            if time_since_progress > 300:
                error_msg = (
                    f"Process hang detected: No progress for {int(time_since_progress)}s. "
                    f"Progress: {current_progress}/{total_num_articles}. "
                    f"{len(finished_futures)}/{len(futures)} processes finished, {running_count} still running. "
                    "This may be due to Windows multiprocessing issues. Try reducing --num_workers or use --num_workers=1."
                )
...
def pipeline_main(  # noqa: C901
    ...
    # =========================================================
    # Case 1: large file shuffle specified (cross-platform)
